...
same cores. Overlap has one side effect: if the background self-play finishes first, the
trainer of cycle $N$ also sees the new shard. This is harmless, since it is newer data from
the same champion, but it makes cycles less reproducible, so controlled experiments should
turn it off.
...
$\alpha=\beta=0.05$, which puts the bounds at $\pm 2.94$. Two practical details:
\begin{itemize}
  ...
  \item \emph{Cap.} The gate plays at most 400 games. If the test is still undecided
...
batches of 40, cap 400) and the free-runner configuration of \cref{sec:infra-actions}
($\Delta_1=40$, batches of 24, cap 192).}
...
\begin{itemize}
  \item \emph{Error control.} SPRT keeps the false-promotion rate close to its nominal
        level: $2.7\%$ at $\Delta=0$, against v3's $8.0\%$. It also has more power for
        real gains: at $\Delta=+50$ it promotes $82\%$ of candidates, against $67\%$.
  \item \emph{Not uniformly cheaper.} Clearly worse candidates are rejected sooner than
        in v3 (105 against 142 expected games at $-100$), and very strong ones are
        accepted sooner at $+200$. But near the indifference region SPRT plays about twice
        as many games (348 against 175 at $\Delta=0$). The v4 gate is an error-control
        improvement, not a saving in games.
  \item \emph{The cap matters.} With the default cap of 400 games, a candidate that is
        truly $+35$ Elo stronger is promoted only $52\%$ of the time, far below the
        nominal $1-\beta=0.95$, because many such tests reach the cap undecided. The
        free-runner configuration's cap of 192 is weaker still ($28\%$ at $+35$). On a
...
A gate can pass by chance ($2.7\%$ at $\Delta=0$ in \cref{tab:orch-gate-oc}), or a
