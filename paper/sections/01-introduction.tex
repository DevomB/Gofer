...
\begin{itemize}
  ...
  \item \textbf{Five engine defects, found by building the gate.} A stale pointer that
        stopped the search from leaving the root on $9\times9$; a first-play-urgency
        constant that collapsed the visit distribution---the policy training target---onto
        one move; an inverted value sign that made more search strictly weaker; a search
        that never saw a game end; and an arena whose per-game seeds were correlated with
        the colour swap. In a controlled match the side searching three times harder went
        from 0 wins in 40 games to 24 (\cref{sec:pipeline-found}). This is the strongest
        argument in this report for building the measurement apparatus before spending
        money on compute.
