...
\begin{itemize}
  ...
  \item \textbf{Three engine defects, found by building the gate.} An inverted value sign
        in search selection, a search that never saw the end of a game, and an arena whose
        per-game seeds were correlated with the colour swap. The first made more search
        strictly weaker: on the project's own baseline, the side searching three times
        harder went from 7 wins in 200 games to 133 (\cref{sec:pipeline-found}). This is
        the strongest argument in this report for building the measurement apparatus
        before spending money on compute.
