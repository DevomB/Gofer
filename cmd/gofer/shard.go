...
// Gofer shards are NumPy .npz archives; see docs/training-data-format.md.
const (
	ShardFormat  = "gofer-shard"
	ShardVersion = 1

	// maxShardBytes bounds in-memory shard construction.
...
type ShardMeta struct {
	...
	// EvalFallbackRate is the heuristic share of model evaluations.
	EvalFallbackRate float64 `json:"eval_fallback_rate"`
	// PassShare is the opening pass mass in full-search policy targets.
...
// writeFileAtomic replaces path with a complete sibling temporary file.
...
func buildShardArrays(samples []Sample) ([]npyArray, int, int, error) {
	...
	// Avoid overflowing capacities or exhausting memory during shard construction.
	...
	for i, s := range samples {
		...
		// Ownership is unmasked by the learner, so every shard row must provide it.
